...
@commands.slash_command()
async def traceroute(inter, *, url):
    ...
    try:
        if "." in url:
            ...
            if " " in url:
                url = url.split(" ")[0]
            await inter.response.defer()
            await run_command_shell("whois " + url)
            if len(out) > 1024:
                link = await paste(out)
                await inter.send(
                    inter.author.mention,
